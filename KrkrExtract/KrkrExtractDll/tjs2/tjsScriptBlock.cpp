...
void tTJSScriptBlock::Dump() const
{
	...
	auto j = InterCodeContextList.begin();

	std::map<tjs_uint, ttstr> ObjMap;

	while (j != InterCodeContextList.end())
	{
		ttstr objname;
		tjs_char hexname[200];

		
		if (wcscmp((*j)->GetName(), L"(getter)") == 0)
		{
			TJS_sprintf(hexname, L"getter_%08x", (*j));
			objname = ttstr((*j)->GetContextTypeNameBegin()) + L"." + ttstr(hexname);
		}
		else if (wcscmp((*j)->GetName(), L"(setter)") == 0)
		{
			TJS_sprintf(hexname, L"setter_%08x", (*j));
			objname = ttstr((*j)->GetContextTypeNameBegin()) + L"." + ttstr(hexname);
		}
		else if (wcscmp((*j)->GetName(), L"(anonymous)") == 0)
		{
			TJS_sprintf(hexname, L"anonymous_%08x", (*j));
			objname = ttstr((*j)->GetContextTypeNameBegin()) + L"." + ttstr(hexname);
		}
		else
		{
			objname = ttstr((*j)->GetContextTypeNameBegin()) + L"." + ttstr((*j)->GetName());
		}
		
		ObjMap.insert(std::make_pair((tjs_uint)(*j), objname));
		j++;
	}
	...
	while(i != InterCodeContextList.end())
	{
		...
		ConsoleOutput(TJS_W("\r\n"), (void*)this);
		ConsoleOutput(L"\\\\==========================================", (void*)this);

		if (wcscmp((*i)->GetName(), L"(getter)") == 0)
		{
			TJS_sprintf(hexname, L"getter_%08x", (*i));
			objname = ttstr((*i)->GetContextTypeNameBegin()) + L"." + ttstr(hexname);
		}
		else if (wcscmp((*i)->GetName(), L"(setter)") == 0)
		{
			TJS_sprintf(hexname, L"setter_%08x", (*i));
			objname = ttstr((*i)->GetContextTypeNameBegin()) + L"." + ttstr(hexname);
		}
		else if (wcscmp((*i)->GetName(), L"(anonymous)") == 0)
		{
			TJS_sprintf(hexname, L"anonymous_%08x", (*i));
			objname = ttstr((*i)->GetContextTypeNameBegin()) + L"." + ttstr(hexname);
		}
		else
		{
			objname = ttstr((*i)->GetContextTypeNameBegin()) + L"." + ttstr((*i)->GetName());
		}

		ConsoleOutput(objname.c_str(), (void*)this);

		(*i)->Disassemble(ConsoleOutput, (void*)this, ObjMap);
		ConsoleOutput((ttstr((*i)->GetContextTypeNameEnd())).c_str(), (void*)this);
